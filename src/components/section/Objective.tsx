    ...
    return (
        <div className="h-auto w-full bg-green-50 relative pb-10 overflow-hidden">
            <div className="flex items-center justify-between flex-col space-y-8 py-12 px-4 lg:px-0 max-w-7xl mx-auto z-10 relative">
                <h2 className="text-3xl md:text-6xl tracking-tight font-bebas text-center max-w-4xl">
                    A natureza não precisa de mais vozes. <br /> Precisa de mais <span className="text-green-400 relative inline-block">consciência</span>.
                </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
                    {cards.map((card, idx) => (
                        <div
                            key={idx}
                            className="bg-white p-6 rounded-3xl shadow-lg border-b-4 border-green-500 transition-transform duration-300 flex flex-col items-center text-center space-y-4"
                        >
                            <div className="p-4 bg-green-50 rounded-full text-green-600 transition-colors duration-300">
                                <card.icon size={32} />
                            </div>
